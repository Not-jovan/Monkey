  ...
  return {
    ...
    attributes: {
      ...
      ...(input.attempt.usage?.outputTokens !== undefined
        ...
        : {}),
      // Renamed on the way in: RunUsage speaks the provider's vocabulary,
      // span attributes speak the trace's. `cachedTokens` is what applyUsage
      // writes for a runtime run and what the trace page reads, so an
      // auditor's cache hit has to arrive under the same name to be seen.
      ...(input.attempt.usage?.cachedInputTokens !== undefined
        ? { cachedTokens: input.attempt.usage.cachedInputTokens }
        : {}),
