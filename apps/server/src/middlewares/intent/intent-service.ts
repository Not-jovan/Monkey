...
export class IntentService {
  ...
  private classifyFailed: ((failure: ClassifyFailure) => void) | null = null;

  constructor(private readonly deps: IntentServiceDeps) {}

  /**
   * Registers the sink for a message whose classification never produced a
   * verdict. A log line is not enough: the user believes the rule they just
   * stated is in force, and without a finding nothing anywhere says it never
   * landed. Registered rather than constructed because the audit middleware
   * owns that report and is built after this service, which it depends on.
   */
  onClassifyFailed(handler: (failure: ClassifyFailure) => void) {
    this.classifyFailed = handler;
  }
